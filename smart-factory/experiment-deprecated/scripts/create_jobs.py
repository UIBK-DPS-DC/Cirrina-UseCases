# pip install argparse commentjson kazoo
import commentjson
import argparse
import json
...
if __name__ == "__main__":
    ...
    parser.add_argument("--host", type=str, default="127.0.0.1:2181", help="ZooKeeper host address")
    parser.add_argument("--csml_file", type=str, required=True, help="Path to the CSML file")
    parser.add_argument("--services_file", type=str, required=True, help="Path to the service descriptions JSON file")
    parser.add_argument("--local_data", type=json.loads, default='{}', help="Local data map in JSON format")
    parser.add_argument("--instances", type=int, default=1, help="Amount of instances for each state machine")
    ...
    sm_names = list(map(lambda sm: sm["name"], \
        filter(lambda sm: "abstract" not in sm or not sm["abstract"], csm["stateMachines"])))
    ...
    def create_job_description(state_machine_name: str) -> dict:
        return {
            ...
            "localData": args.local_data[state_machine_name] if state_machine_name in args.local_data else {},
    ...
    try:
        ...
        for i in range(args.instances):
            for job_num, sm_name in enumerate(sm_names):
                ...
                job_description_bytes: bytes = json.dumps(job_description).encode("utf-8")
